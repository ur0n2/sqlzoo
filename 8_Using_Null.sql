...
SELECT name, COALESCE(teacher.mobile, '07986 444 2266') as Phone FROM teacher
...
SELECT dept.name, COUNT(teacher.name) FROM teacher RIGHT OUTER JOIN dept ON dept.id=teacher.dept GROUP BY dept.name

--# 9. Use CASE to show the name of each teacher followed by 'Sci' if the the teacher is in dept 1 or 2 and 'Art' otherwise.
SELECT teacher.name, CASE WHEN dept.id = '1' or dept.id = '2' THEN 'Sci' ELSE 'Art' END FROM teacher LEFT JOIN dept ON dept.id=teacher.dept
...
SELECT teacher.name, CASE WHEN dept.id = '1' or dept.id = '2' THEN 'Sci' ELSE 'None' END FROM teacher LEFT JOIN dept ON dept.id=teacher.dept